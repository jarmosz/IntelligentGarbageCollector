...
def generate_roads():
#maybe add road nodes to separate list
    w=0
    roads_vertically = randint(MIN_RANDOMED_ROADS_VERTICALLY,MAX_RANDOMED_ROADS_VERTICALLY)
    for i in range (0,roads_vertically):
        ...
        if grid[0][column].upper_node != None or grid[0][column-1].upper_node != None or grid[0][column+1].upper_node != None:
            ...
            for j in grid:
                ...
                current_node.position = Vector2(grid.index(j)*CELL_SIZE,column*CELL_SIZE)  
                j[column].upper_node = current_node
                w = w+1
                roads.append(current_node)
                del current_node
    roads_horizontally = randint(MIN_RANDOMED_ROADS_HORIZONTALLY,MAX_RANDOMED_ROADS_HORIZONTALLY)
    for i in range(0,roads_horizontally):
        ...
        if grid[row][0].upper_node != None or grid[row-1][0].upper_node != None or grid[row+1][0].upper_node != None:
            ...
            for j in grid[row]:
                ...
                current_node.position = Vector2(row*CELL_SIZE,grid[row].index(j)*CELL_SIZE)
                w = w+1
                roads.append(current_node)
def render_color(color,node):
    pygame.draw.rect(screen, color, pygame.Rect(node.position.x, node.position.y, CELL_SIZE,CELL_SIZE))
